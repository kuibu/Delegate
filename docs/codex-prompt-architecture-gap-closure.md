...
| 4 | Permission system | Partial, governance visibility stronger | `allow / ask / deny`, capability rules, explicit resource-scope checks, path/domain/cost/paid-plan gates, dashboard-editable compute defaults, Delegate-managed overlays, owner-managed baseline overlays, trusted-customer trust-tier overlays, conversation-scoped compute entitlements, organization baseline overlays, customer-account overlays, contact-to-customer assignment, and team/customer approval insights now exist | Widen org/customer governance beyond the compute lane into artifacts, deliverables, billing, and broader approval scopes | The compute lane now has a much clearer governance surface, but the same org/customer semantics do not yet consistently cover the rest of the control plane | P1 |
...
1. `P5-C` unified deliverable analytics and owner-facing packaging presets
2. `P6-A` deeper org/customer governance beyond the compute lane

Why this order:

- Approval and audit visibility now covers team/customer views inside the compute lane, so the next highest-value gap shifts to how packaged deliverables are measured, reused, and productized
- Delegate can now package and publish deliverables and scope compute policy by customer account, which means the next trust problem is extending that governance model beyond compute into the rest of the control plane
