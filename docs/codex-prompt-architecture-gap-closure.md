...
| 4 | Permission system | Partial, broader governance landed | `allow / ask / deny`, capability rules, explicit resource-scope checks, path/domain/cost/paid-plan gates, dashboard-editable compute defaults, Delegate-managed overlays, owner-managed baseline overlays, trusted-customer trust-tier overlays, conversation-scoped compute entitlements, organization baseline overlays, customer-account overlays, and contact-to-customer assignment now exist | Add richer approval and audit presentation across teams/customers, plus wider governance surfaces beyond the compute lane | The repo now has real org/customer governance above owner overlays, but approval UX and broader multi-actor visibility still lag the policy model | P1 |
...
| 8 | Files and artifacts | Partial, productized | Object storage, retention, representative/contact/session attribution, detail/download APIs, dashboard artifact viewer, pinned artifacts, download tracking, artifact egress ledger entries, bundle packaging, public deliverables, and unified material/file download routes now exist | Add richer deliverable filtering, packaging presets, and broader generated-document ergonomics | The core deliverable workflow now exists, but owner-facing packaging presets and higher-level file operations are still lighter than the target system | P1 |
...
1. `P5-B` richer approval and audit presentation across teams/customers
2. `P5-C` unified deliverable analytics and owner-facing packaging presets
3. `P6-A` deeper org/customer governance beyond the compute lane

Why this order:

- The governance layer is now broader than simple owner overlays, so the next highest-value gap shifts to how approvals and audit traces are presented to broader teams/customers
- Delegate can now package and publish deliverables and scope compute policy by customer account, which means the next trust problem is no longer raw access control but visibility, review, and accountability across actors
- After that, the deliverable analytics and packaging experience can catch up to the richer governance model
