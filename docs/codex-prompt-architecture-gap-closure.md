...
| 8 | Files and artifacts | Partial, productized and measured | Object storage, retention, representative/contact/session attribution, detail/download APIs, dashboard artifact viewer, pinned artifacts, download tracking, artifact egress ledger entries, cached bundle packaging, public deliverables, deliverable analytics, and owner-facing packaging presets now exist | Extend org/customer governance semantics across artifact and deliverable permissions, then keep improving generated-document ergonomics | Deliverables are now measurable and easier to package, but the richer governance model still stops mostly at the compute lane | P1 |
...
1. `P6-A` deeper org/customer governance beyond the compute lane
2. `P6-B` unify approval, artifact, deliverable, and billing governance semantics

Why this order:

- Deliverables now have analytics, cached package artifacts, and preset-driven packaging, so the next highest-value gap is enforcing the same org/customer governance semantics outside the compute lane
- Delegate can already package and publish deliverables and scope compute policy by customer account, which means the next trust problem is extending those governance boundaries into artifacts, deliverables, approvals, and billing controls together
