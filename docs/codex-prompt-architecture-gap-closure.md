...
| 4 | Permission system | Partial, governed-action model landed | `allow / ask / deny`, capability rules, explicit resource-scope checks, path/domain/cost/paid-plan gates, dashboard-editable compute defaults, Delegate-managed overlays, owner-managed baseline overlays, trusted-customer trust-tier overlays, conversation-scoped compute entitlements, organization baseline overlays, customer-account overlays, contact-to-customer assignment, team/customer approval insights, dashboard-visible resource governance across artifacts and deliverables, and a unified governed-action snapshot across approval/resource/billing semantics now exist | Deepen owner/team-facing governed-action presentation and escalation rules | The repo now has a shared governed-action language, but the next gap is richer escalation ownership, actor semantics, and higher-signal operational views on top of it | P1 |
...
| 8 | Files and artifacts | Partial, productized and governed | Object storage, retention, representative/contact/session attribution, detail/download APIs, dashboard artifact viewer, pinned artifacts, download tracking, artifact egress ledger entries, cached bundle packaging, public deliverables, deliverable analytics, owner-facing packaging presets, resource-governance snapshots, and governed-action linkage across artifacts/deliverables/billing now exist | Keep tightening package lifecycle ergonomics and richer owner/team semantics on governed resource actions | Deliverables are now measurable, resource-governed, and linked into the broader action model, but packaging lifecycle ergonomics and escalation semantics can still get clearer | P1 |
...
- Dashboard/control plane:
  ...
  - `packages/web-data/src/governed-actions.ts`
- Data model:
...
1. `P6-C` deepen owner/team-facing governed-action presentation and escalation rules
2. `P6-D` tighten packaging / approval / billing escalation semantics on top of the governed-action console

Why this order:

- The repo now exposes a governed-action console, so the next highest-value gap is making ownership, escalation, and cross-team operational views sharper on top of that shared language
- Delegate can already package, publish, classify, and bill resources by customer account, which means the next trust problem is deciding which owner/team semantics need to be more explicit before any larger IAM move
...
- model runtime and MCP transport have first safe slices, and Temporal-backed workflow start, durable timer waiting, outbox dispatch, cancellation delivery, and phase-aware dashboard state now exist for the two current durable workflow kinds
